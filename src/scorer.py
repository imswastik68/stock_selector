...
# DISQUALIFIER_WEIGHTS by key, see _compute_score. A key here only takes effect if
# it matches a live signal-map key exactly (src/scorer.py:_build_signal_map) —
# 'volume_surge'/'distribution' below are backtest-only column names and never
# fired in production; live keys are 'volume_5x'/'distribution_signal'.
...
REGIME_WEIGHTS: dict[str, dict[str, int] | None] = {
    "uptrend":   None,
    "ranging":   None,  # reverted 2026-07 (was never validated as actually shipped — see below)
    "downtrend": None,  # reverted 2026-07 (was never validated as actually shipped — see below)
}

# Prior calibration (shipped 2026-06-14, commit 9f4b89e). Kept only for Phase 5
# comparison — do not re-ship as-is. It was validated under REPLACE semantics
# (silently zeroed bulk_deal_fii_dii/actual_52w_breakout/delivery_surge/etc in
# ranging+downtrend, a behavior the live scorer never actually exercised
# end-to-end) and with keys (volume_surge/distribution) that don't match live
# signal names, so it was inert in production regardless of the above. It never
# operated as calibrated; reverted to flat weights until Phase 5 recalibrates
# under merge semantics with correct keys.
# _PRIOR_REGIME_WEIGHTS_REPLACE_SEMANTICS = {
#     "ranging":   {"rsi_momentum": -3, "rs_vs_nifty": -1, "rsi_bearish_div": -3, "rsi_bullish_div": 5, "macd_bearish_cross": 5, "bb_squeeze_breakout": 4, "bullish_candle": 4, "bearish_candle": 0, "rs_quality_strong": -5, "volume_surge": 2, "distribution": 4},
#     "downtrend": {"rsi_momentum": 5, "rs_vs_nifty": 3, "rsi_bearish_div": 4, "rsi_bullish_div": -5, "macd_bearish_cross": -3, "bb_squeeze_breakout": -5, "bullish_candle": -1, "bearish_candle": -4, "rs_quality_strong": 2, "volume_surge": -5, "distribution": -5},
# }
